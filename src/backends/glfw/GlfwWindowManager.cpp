...
GlfwWindowManager::GlfwWindowManager(const WindowDetails &windowDetails, GraphicsAPI graphicsAPI)
    : window_(nullptr), metalLayer_(nullptr), windowDetails_(windowDetails), windowFlags_{}, graphicsAPI_(graphicsAPI),
      keyboardListener_(std::make_unique<GLFWKeyboardListener>()),
      mouseListener_(std::make_unique<GLFWMouseListener>()), userData_(std::make_unique<GlfwUserData>()),
      windowShouldClose_(false), ownsInit_(false)
{
...
GlfwWindowManager::GlfwWindowManager(GlfwWindowManager &&other) noexcept
    : window_(std::exchange(other.window_, nullptr)), metalLayer_(std::exchange(other.metalLayer_, nullptr)),
...
void GlfwWindowManager::createWindow(const char *windowName)
{
    ...
    case GraphicsAPI::CPU:
        ...
        throw GraphicsException("GLFW has no software rendering path; use SDL3/X11/Wayland for "
    ...
    window_ = glfwCreateWindow(windowDetails_.width, windowDetails_.height, windowName,
                               windowDetails_.fullscreen ? glfwGetPrimaryMonitor() : nullptr, nullptr);
    ...
    if (graphicsAPI_ == GraphicsAPI::Metal)
    {
        ...
        if (!metalLayer_)
        {
            ...
            window_ = nullptr;
            throw GraphicsException("Failed to attach a CAMetalLayer to the GLFW window's content view");
        }
    }
#endif
...
const std::vector<const char *> GlfwWindowManager::getVulkanExtensions() const
{
    ...
    if (!extensions)
    {
        throw GraphicsException("Failed to get Vulkan extensions from GLFW (Vulkan loader unavailable?)");
    }
