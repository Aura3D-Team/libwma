...
const wl_keyboard_listener WaylandKeyboardListener::keyboardListener_ = {.keymap = handleKeymapCallback,
...
WaylandKeyboardListener::WaylandKeyboardListener(WindowFlags *flags)
    : KeyboardListener(), keyboard_(nullptr), windowFlags_(flags)
{
}
...
void WaylandKeyboardListener::handleKeymap(u32 format, i32 fd, u32 size)
{
    ...
    xkbKeymap_ = xkb_keymap_new_from_string(xkbContext_, static_cast<const char *>(mapped), XKB_KEYMAP_FORMAT_TEXT_V1,
    ...
    composeTable_ = xkb_compose_table_new_from_locale(xkbContext_, locale, XKB_COMPOSE_COMPILE_NO_FLAGS);
    if (composeTable_)
 ...
 */
template <typename Fill, typename Emit> void decodeXkbUtf8(Fill &&fill, Emit &&emit)
{
    ...
    utf8::decode(std::string_view{heapBuffer.data(),
                                  (static_cast<usize>(written) < length) ? static_cast<usize>(written) : length},
                 emit);
}
...
void WaylandKeyboardListener::handleModifiers(u32, u32 mods_depressed, u32 mods_latched, u32 mods_locked, u32 group)
{
    /*
...
void WaylandKeyboardListener::handleKeymapCallback(void *data, wl_keyboard *, u32 format, i32 fd, u32 size)
{
...
void WaylandKeyboardListener::handleEnterCallback(void *data, wl_keyboard *, u32 serial, wl_surface *surface,
                                                  wl_array *keys)
{
    auto *listener = static_cast<WaylandKeyboardListener *>(data);
    if (listener)
...
void WaylandKeyboardListener::handleLeaveCallback(void *data, wl_keyboard *, u32 serial, wl_surface *surface)
{
    auto *listener = static_cast<WaylandKeyboardListener *>(data);
    if (listener)
...
void WaylandKeyboardListener::handleKeyCallback(void *data, wl_keyboard *, u32 serial, u32 time, u32 key, u32 state)
{
    auto *listener = static_cast<WaylandKeyboardListener *>(data);
    if (listener)
...
void WaylandKeyboardListener::handleModifiersCallback(void *data, wl_keyboard *, u32 serial, u32 mods_depressed,
...
void WaylandKeyboardListener::handleRepeatInfoCallback(void *data, wl_keyboard *, i32 rate, i32 delay)
{
    auto *listener = static_cast<WaylandKeyboardListener *>(data);
    if (listener)
